...
contract WeeklyFrinkLottery is Managed {
    ...
    constructor(address _tokenAddress) public {
        ERC20TokenFrink tokenContract = ERC20TokenFrink(tokenAddress);
    }

    function pickWeeklyLotteryWinner() public onlyManager {
        ...
        lastWinner = winner;
        winners.push(winner);
        uint256 lotteryBalance = tokenContract.balanceOf(address(this));
        winnerDetails[winner] = lotteryBalance;

        require(
            tokenContract.transfer(winner, lotteryBalance),
    ...
    function pickWinner() private returns (address) {
        uint256 index = random() % getPlayers().length;
