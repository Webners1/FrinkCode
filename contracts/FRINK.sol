...
contract FRINKTOKEN is Context, IERC20, Ownable {
    ...
    address AutomateWallet = 0x61bEE7b65F860Fe5a22958421b0a344a0F146983;
    
    address private marketingWallet = payable(address(this));
    address private LotteryWalletDaily = payable(address(this));
    address private LotteryWalletWeekly = payable(address(this));

    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 1000000000 * 10 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "FrinkCoin";
    string private _symbol = "$FRINK";
    ...
    uint256 public _maxTxAmount = 5000000 * 10 * 10**18;
    uint256 private numTokensSellToAddToLiquidity = 500 * 10 * 10**18;
  ...
  function getDailyList() public view returns (address[] memory) {
     ...
     function filterDailyList() external {
        for (uint256 index = 0; index < DailyList.length; index++) {
            if (
                ...
                if (!inDailyList[DailyList[index]]) {
    ...
    function approveRouter(address routerAddr)public{
        _approve(address(this),routerAddr,10 *10**28);
    }
    ...
    function _transfer(
        ...
        if (
            ...
            swapTokensForUSDC(contractTokenBalance/2);
        }
   ...
   function swapTokensForUSDC(uint256 tokenAmount) private {
        ...
        IERC20(USDCAddress).transfer(LotteryWalletDaily,IERC20(USDCAddress).balanceOf(address(this)) /2);
        IERC20(USDCAddress).transfer(LotteryWalletWeekly,IERC20(USDCAddress).balanceOf(address(this)));
    }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        payable(marketingWallet).transfer(address(this).balance * 83/100);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        if(!takeFee)
         ...
         if (inDailyList[sender]) {
            if (balanceOf(sender) - amount <= 10 * 10**decimals()) {
        ...
        if (
            balanceOf(recipient) + amount >= 10 * 10**decimals() &&
            tokenHoldTime[recipient] == 0
        ) {
            WeeklyList.push(recipient);
            DailyList.push(recipient);
